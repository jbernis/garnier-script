#!/usr/bin/env python3
"""
Script pour traiter les variants Cristel en attente (extraire prix, SKU, stock).
"""
    ...
    try:
        ...
        for variant_data in details.get('variants', []):
            ...
            if variant_data_code == code_vl or variant_data.get('code_vl') == code_vl:
                ...
                variant_gencode = variant_data.get('gencode') or variant_data.get('gtin') or ''
                variant_price_pvc = variant_data.get('pvc') or variant_data.get('price_pvc') or variant_data.get('price') or ''
                variant_price_pa = variant_data.get('pa') or variant_data.get('price_pa') or None
                variant_size = variant_data.get('size') or variant_data.get('size_text') or ''
                variant_color = variant_data.get('color') or ''
                variant_stock = variant_data.get('stock') or variant_data.get('inventory') or None
                ...
                if missing_fields:
                    ...
                    # Toutes les données essentielles sont présentes, marquer comme completed
                    db.update_variant_data(
                        variant_id=variant_id,
                        sku=variant_sku,
                        gencode=variant_gencode if variant_gencode else None,
...
def process_urls(status='pending', limit=None, retry_errors=False, output_db='cristel_products.db', headless=True, category=None, categories=None, subcategory=None):
    """
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Traite les variants Cristel en attente'
    )
    parser.add_argument(
        '--status',
        default='pending',
