#!/usr/bin/env python3
"""
Script orchestrateur pour traiter une sous-catégorie Cristel complète.
...
spec = importlib.util.spec_from_file_location("cristel_collect", collect_path)
...
spec = importlib.util.spec_from_file_location("cristel_process", process_path)
...
spec = importlib.util.spec_from_file_location("cristel_generate", generate_path)
...
def main():
    parser = argparse.ArgumentParser(
        description='Traite une sous-catégorie Cristel complète (collect → process → generate-csv)'
    )
    parser.add_argument(
        '--url', '-u',
        required=True,
